...
public:
  void SetUp(benchmark::State & st)
  {
    ...
    publisher = node->create_publisher<test_msgs::msg::Empty>("empty_msgs", rclcpp::QoS(10));
...
BENCHMARK_F(PerformanceTestExecutor, single_thread_executor)(benchmark::State & st)
{
  ...
  for (auto _ : st) {
    executor.spin_some(std::chrono::nanoseconds(50));
  }
}
...
BENCHMARK_F(PerformanceTestExecutor, static_single_thread_executor)(benchmark::State & st)
{
  ...
  for (auto _ : st) {
    auto ret = executor.spin_until_future_complete(shared_future, std::chrono::milliseconds(1));
...
BENCHMARK_F(PerformanceTestExecutor, spin_until_future_complete)(benchmark::State & st)
{
  // test success of an immediately finishing future
  std::promise<bool> promise;
  std::future<bool> future = promise.get_future();
...
BENCHMARK_F(PerformanceTestExecutor, static_executor_entities_collector)(benchmark::State & st)
{
